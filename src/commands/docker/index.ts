...
export const dokcerFunction = async (args: any, method: Method) => {
  if (!testIfDockerExist) {
    program.log(color.red("Dock is not installed"));
    return;
  } else {
    ...
    const elementsList: string[] = (
      await execute(listCommand)).stdout
      .trim()
      .split(/ |\n/)
    ...
    if (args.options.forced) {
      await Promise.all(elementsList.map(async (elem: any) => {
      ...
      await Promise.all(
        elementsList.map(async (elem: any) => {
      ...
      (await execute(listCommand)).stdout.length === 0
        ...
        : spinner.fail(
          ...
          color.rgb(31, 188, 193)(`dops docker ${actionType} `) +
          color.blue("[options]") +
